...
def moss_tts_masked_embedding(embedding: nn.Embedding,
    ...
    """
    Embed input_ids where they differ from ignore_index; return zero vectors elsewhere.

    Args:
        embedding: An nn.Embedding layer.
        input_ids: A LongTensor of any shape, possibly containing ignore_index.
        ignore_index: The index marking positions to ignore (default: -100).

    Returns:
        embeddings: A tensor of shape (*input_ids.shape, embedding.embedding_dim).
    """
    # mask: True means a normal embedding; False means a zero vector.
    mask = (input_ids != ignore_index)  # shape: [...]

    # Temporarily replace invalid indices such as -100 before the embedding lookup.
    ...
    # Zero the positions corresponding to ignore_index.
    out[~mask] = 0.0
...
class CustomMixin(GenerationMixin): # TODO: Verify correctness.
    def _sample(
        self,
        ...
        **model_kwargs,
        ...
        if generation_config.do_samples is not None:
            ...
            for i, layer_config in enumerate(generation_config.layers):
                if not do_samples[i]:
                    continue
                if layer_config.get("repetition_penalty") is not None and i != 0: # The text layer does not use a repetition penalty.
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            local_transformer_inputs = torch.zeros(batch_size, 0, local_trm_dim).to(device).to(dtype) # (B, 0 <= t <= Nq, D), Maintain the current local transformer inputs.
            current_local_transformer_input = self.speech_embedding_to_local_mlp(global_trm_output_hidden_states) # (B, D) Maintain the local transformer input at the current timestamp.
        ...
        if return_dict_in_generate:
            ...
            start_lengths = input_ids_length - start_indices - 1 # Zero for voice cloning; prompt audio length for continuation, excluding audio_start_token.
            output = []
...
class MossTTSDelayModel(MosiTTSPretrainedModel, CustomMixin):
    ...
    def _prepare_shifted_audio_inputs(self, label_ids): # (B, T, 1 + Nq) May contain -100.
    ...
    def forward(
        ...
        inputs_embeds: Optional[torch.FloatTensor] = None,
        labels: Optional[torch.LongTensor] = None, # (B, T, 1 + Nq), TODO: labels are input_ids shifted by one position.
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            local_transformer_output_hidden_states = self.local_transformer( # TODO: Positional encoding is not enabled.
                input_ids=None,
                attention_mask=None,
