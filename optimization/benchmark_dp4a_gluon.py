"""Explicit Gluon layout tuning against the preceding direct-load plan."""
import json
import hashlib
import torch
import triton
...
from .dp4a_gluon import linear,_gluon_gemv
...
def main():
    ...
    for name in ('qkv','out','up','down'):
        ...
        for group_contiguous in (1,2,4,8):
            for rows in (1,2,4):
                for warps in (1,2,4):
                    actual=linear(x,w,s,rows=rows,warps=warps,paired=paired,group_contiguous=group_contiguous)
                    ...
                    item={'group_contiguous':group_contiguous,'rows':rows,'warps':warps,'us':measure(lambda pair:linear(x,*pair,rows=rows,warps=warps,paired=paired,group_contiguous=group_contiguous),ring),
        ...
        for layer in (0,17,35):
            ...
            for x in [xs[i:i+1].cuda() for i in (0,231,528,1186)]+[zero,spike]:
                ...
                for c in case['candidates']:
                    ...
                    with torch.cuda.stream(stream):actual=linear(x,w,s,rows=c['rows'],warps=c['warps'],paired=paired,group_contiguous=c['group_contiguous'])
        ...
        if case['best']:
            ...
            kernel=_gluon_gemv[(triton.cdiv(on,c['rows']),)](q,sx,w,s,y,on,k,triton.next_power_of_2(k//32),c['rows'],paired,c['group_contiguous'],c['warps'],num_warps=c['warps'])
            c['registers']=kernel.n_regs;c['spills']=kernel.n_spills;c['shared_bytes']=kernel.metadata.shared
            for ext in ('ptx','ttgir'):(RESULTS/f'gluon_{name}.{ext}').write_text(kernel.asm[ext])
            del q,sx,y
        results['cases'].append(case)
        print(name,'REFERENCE',case['reference_us'],'EXACT BEST',case['best'],'FASTEST',case['fastest'],flush=True)
        (RESULTS/'dp4a_gluon_kernels.json').write_text(json.dumps(results,indent=2)+'\n')
    ...
    for case in results['cases']:
        ...
        if best and best['us']<case['reference_us']*.98:
            plan['projections'][case['projection']].update(rows=best['rows'],warps=best['warps'],activation_load='gluon',group_contiguous=best['group_contiguous'])
    plan['gluon_tuning_sha256']=hashlib.sha256((RESULTS/'dp4a_gluon_kernels.json').read_bytes()).hexdigest()
    plan['selection']='Explicit Gluon layout only where 18/18 exact comparisons pass and median operator gain exceeds 2%; other projections use the previous matching plan. Full-model verification recorded separately.'
    (RESULTS/'dp4a_gluon_exact_plan.json').write_text(json.dumps(plan,indent=2)+'\n')
