...
def main():
    ...
    path=RESULTS/f'bulk_prefetch_validation_{a.tag}.json';assert not path.exists(),'Preserve evidence'
    e=load_layer(17);checks=[];guards=[];stream=torch.cuda.Stream();stream.wait_stream(torch.cuda.current_stream())
    probes=[{'divisor':1,'scales':True,'policy':1},{'divisor':16},{'divisor':1,'ahead':64}]
