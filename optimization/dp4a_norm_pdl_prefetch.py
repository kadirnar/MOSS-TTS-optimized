"""PDL with independent projection weights/scales loaded before the grid wait."""
import torch
import triton
...
@gluon.jit
def _projection(a,b,xscale,W,S,base,N:gl.constexpr,R:gl.constexpr,I:gl.constexpr,F:gl.constexpr,PW,PS,PRE:gl.constexpr):
    ...
    if PRE&2:w=PW
    else:w=gl.load(gl.cast(W,gl.pointer_type(gl.uint32))+ri[:,None,None]*512+pos,ri[:,None,None]<N,0)
    ...
    if PRE&1:scale=PS
    else:scale=gl.load(S+rf[:,None]*128+gf[None,:],rf[:,None]<N,0).to(gl.float32)
...
def _kernel(X,RES,NW,W,S,SUM,Y,OQ,OS,NY,NQ,NS,N:gl.constexpr,R:gl.constexpr,
            ...
            DEBUG:gl.constexpr,PDL:gl.constexpr,TRIGGER:gl.constexpr,PRE:gl.constexpr):
    ...
    if PRE&2:
        ...
        c=gl.arange(0,4,layout=gl.SliceLayout(0,gl.SliceLayout(0,I)))
        pos=gi[None,:,None]*4+gl.expand_dims(gl.expand_dims(c,0),0)
        pw=gl.load(gl.cast(W,gl.pointer_type(gl.uint32))+ri[:,None,None]*512+pos,ri[:,None,None]<N,0)
        if FUSED:uw=gl.load(gl.cast(W,gl.pointer_type(gl.uint32))+(ri[:,None,None]+N)*512+pos,ri[:,None,None]<N,0)
    if PRE&1:
        rf=base+gl.arange(0,R,layout=gl.SliceLayout(1,F))
        gf=gl.arange(0,128,layout=gl.SliceLayout(0,F))
        ps=gl.load(S+rf[:,None]*128+gf[None,:],rf[:,None]<N,0).to(gl.float32)
        if FUSED:us=gl.load(S+(rf[:,None]+N)*128+gf[None,:],rf[:,None]<N,0).to(gl.float32)
    if PDL:wait()
    ...
    base=gl.program_id(0)*R
    value=_projection(a,b,scale,W,S,base,N*2 if FUSED else N,R,I,F,pw,ps,PRE).to(gl.bfloat16)
    if FUSED:
        up=_projection(a,b,scale,W,S,base+N,N*2,R,I,F,uw,us,PRE).to(gl.bfloat16).to(gl.float32)
...
def linear(x,residual,norm_weight,eps,w,s,*,fused=False,rows=4,integer_groups=1,integer_rows=1,debug=False,pdl=True,trigger_mode=1,return_kernel=False,prefetch=3):
    if prefetch not in (0,1,2,3):raise ValueError("Invalid preload mode")
    ...
    compiled=_kernel[(triton.cdiv(n,rows),)](x,residual,norm_weight,w,s,summed,y,oq,os,ny,nq,ns,n,rows,eps,residual is not None,fused,integer_groups,integer_rows,debug,pdl,trigger_mode,prefetch,num_warps=4,launch_pdl=pdl)
