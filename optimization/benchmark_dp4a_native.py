...
from .dp4a_native import linear,library,permute_groups
...
def main():
    ...
    outfile=RESULTS/f'dp4a_native_kernels_{args.tag}.json';assert not outfile.exists(),'Preserve old results'
    ...
    for name in ('qkv','out','up','down'):
        ...
        permuted_ring=[permute_groups(*pair) for pair in ring];actual=aa=bb=None
        configs=[{'warps':nw,'repeat':repeat,'cache':cache,'permuted':perm} for nw in ((4,8) if name=='up' else (2,4,8)) for repeat in ((1,) if name=='up' else (1,2,4)) for cache in (False,True) for perm in (False,True)]
        ...
        for c in configs:
            ...
            try:
                actual=candidate(x,*(permuted_ring[0] if c['permuted'] else (w,s)),qx,c)
        ...
        for layer in (0,17,35):
            ...
            for x in [xs[i:i+1].cuda() for i in (0,231,528,1186)]+[zero,spike]:
                ...
                for row in case['candidates']:
                    ...
                    with torch.cuda.stream(stream):actual=candidate(x,*((pw,ps) if row['config']['permuted'] else (w,s)),qx,row['config'])
                    torch.cuda.current_stream().wait_stream(stream)
                    aa=(actual[0],*actual[1]) if name=='up' else (actual,);bb=(expected[0],*expected[1]) if name=='up' else (expected,)
                    count=sum(int((a!=b).sum()) for a,b in zip(aa,bb))
                    row['checks']+=1;row['exact_checks']+=int(count==0);row['mismatches']+=count
            del saved,w,s,pw,ps,xs,x,qx,zero,spike,actual,expected,aa,bb
