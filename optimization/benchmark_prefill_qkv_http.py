"""Sequential temporary loopback servers for ordinary versus fused BF16 prefill QKV preparation.
...
def main():
    ...
    base=[sys.executable,'-u','-m','optimization.server','--port','18084',
          ...
          '--decode-buckets','--attention-quant','--native-attention','--gateup-quant','--scaled-dp4a','--norm-projection','--audio-head-buckets','--short-scales','--projection-pdl','--attention-pdl','--codec-clock','--first-audio-graph','--bulk-prefetch']
    for mode in ('control','prefill'):
        name='http_prefill_qkv_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--prefill-qkv'] if mode=='prefill' else [])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a'] and health['norm_projection'] and health['audio_head_buckets'] and health['short_scales'] and not health['compressed_scales'] and health['projection_pdl'] and health['attention_pdl'] and health['codec_clock'] and health['first_audio_graph'] and health['bulk_prefetch'] and health['prefill_qkv']==(mode=='prefill')
    ...
    a=json.loads((RESULTS/f'http_prefill_qkv_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_prefill_qkv_prefill_{tag}.json').read_text())
    ...
    result={'method':'Sequential temporary servers, qualified bulk-prefix control (QKV address specialization included) then fused BF16 prefill QKV preparation; identical text/reference/seeds 501-520, all 32 codebooks. No concurrent GPU test.','all_20_full_pcm_streams_exact':same,
            'control_ttfa':a['ttfa'],'prefill_ttfa':b['ttfa'],'control_fresh_single_ms':a['fresh_reference_to_pcm_ms'],'prefill_fresh_single_ms':b['fresh_reference_to_pcm_ms'],
            'control_cancellation_statuses':a['cancellation_recovery_statuses'],'prefill_cancellation_statuses':b['cancellation_recovery_statuses'],'temporary_servers_stopped':True}
    (RESULTS/f'http_prefill_qkv_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
