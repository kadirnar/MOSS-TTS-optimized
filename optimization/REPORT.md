# Measured optimization report — 2026-09-19

**50 ms TTFA was not achieved.** The requested checkpoint now streams with voice cloning on one H200 NVL. The all-32-codebook BF16/FP32 path measures **175.2 ms median TTFA**, a **4.84× improvement** over the measured upstream loop. The running local service measures **174.2 ms median / 176.4 ms p95** over 20 complete HTTP requests.
...
| --- | ---: | ---: | ---: |
| Upstream loop + earliest-frame emission | 848.52 | 851.60 | 5 |
| Optimized BF16 LLM / FP32 codec | 175.22 | 177.59 | 5 |
| Optimized loopback HTTP | 174.19 | 176.36 | 20 |
| Uncached reference, optimized encoder | 196.01 | 196.04 | 3 |
| Experimental FP8 MLP, all 32 codebooks | 145.01 | 146.27 | 5 |
...
Service median real-time factor is **0.149**, approximately **6.71× real time** including startup delay for each utterance. Reference registration in the final HTTP test took 22.48 ms for encoding after WAV parsing. The uncached-reference test above starts with a decoded CPU waveform and includes resampling, normalization, encoding, and synthesis to first PCM.
...
6. Uncached reference encoding now uses FP32 graph buckets. In its paired benchmark the encoder fell from **69.07 to 20.89 ms**. It produced identical reference tokens for eight tested durations from 0.24 to 14.5 seconds.
...
| --- | ---: | ---: |
| torch_silu | 3.289 | 0.00000 |
| triton_silu | 1.461 | 0.00000 |
| sglang_silu | 3.498 | 0.03125 |
| vllm_silu | 5.804 | 0.00000 |
| cuda_c_silu | 1.584 | 0.00000 |
| tilelang_silu | 1.505 | 0.00000 |
| cute_dsl_silu | 1.506 | 0.00000 |

Native CUDA/C and multiple Triton GEMV layouts were also measured. For the 24,576×4,096 projection, the selected Triton row kernel measured 49.13 µs versus cuBLAS 58.33 µs and native CUDA/C 62.51 µs. These are shape-specific microbenchmarks, not end-to-end TTFA. TileLang and CuTe DSL kernels compiled, ran, and passed the tested elementwise comparison. The integrated path uses Triton and cuBLAS; native CUDA/C, TileLang, and CuTe trials are retained for reproducibility.

## Correctness and quality evidence

- The 140-frame decoder comparison crosses the ten-second ring boundary and exercises a non-default CUDA stream. SNR against the original decoder was **115.95 dB**, max absolute waveform error **6.3e-06**, and the first-frame error after reset was zero.
...
- Over 36 teacher-forced LLM steps, maximum relative RMS logit error was **0.772%**; active-codebook top-1 agreement averaged **92.18%** and mean active-codebook KL divergence was **0.00315**. Floating-point operation ordering and sampling precision differ. Generation is **not bitwise identical** to the upstream implementation.
...
Repository commit: `934d6826b084c46a0d033402174d5f8ac4ed2519`. TTS revision: `cdd3b911b1585e3f2dbc7775ef10f9926f58850a`. Codec revision: `3cd226ba2947efa357ef453bcad111b6eafba782`. Hardware: one NVIDIA H200 NVL, CUDA 12.8 toolkit, Torch 2.9.1+cu128, Triton 3.5.1. Upstream [model](https://huggingface.co/OpenMOSS-Team/MOSS-TTS-v1.5) and [source](https://github.com/OpenMOSS/MOSS-TTS) are unchanged; changes are isolated in `optimization/` plus the supervisor service files.
