"""Sequential temporary loopback servers for the lossless scale-allocation comparison.
...
def main():
    ...
    for mode in ('control','compressed'):
        name='http_compressed_scales_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--compressed-scales'] if mode=='compressed' else [])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a'] and health['norm_projection'] and health['audio_head_buckets'] and health['compressed_scales']==(mode=='compressed')
    ...
    a=json.loads((RESULTS/f'http_compressed_scales_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_compressed_scales_compressed_{tag}.json').read_text())
    ...
    result={'method':'Sequential temporary servers, control then compressed; identical text/reference/seeds 501-520, all 32 codebooks. No concurrent GPU test.','all_20_full_pcm_streams_exact':same,
            'control_ttfa':a['ttfa'],'compressed_ttfa':b['ttfa'],'control_fresh_single_ms':a['fresh_reference_to_pcm_ms'],'compressed_fresh_single_ms':b['fresh_reference_to_pcm_ms'],
            'control_cancellation_statuses':a['cancellation_recovery_statuses'],'compressed_cancellation_statuses':b['cancellation_recovery_statuses'],'temporary_servers_stopped':True}
    (RESULTS/f'http_compressed_scales_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
