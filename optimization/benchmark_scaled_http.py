"""Sequential temporary loopback servers for the exact INT4 unpack comparison.
...
def main():
    ...
    for mode in ('control','scaled'):
        name='http_scaled_dp4a_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--scaled-dp4a'] if mode=='scaled' else [])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a']==(mode=='scaled')
    ...
    a=json.loads((RESULTS/f'http_scaled_dp4a_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_scaled_dp4a_scaled_{tag}.json').read_text())
    ...
    result={'method':'Sequential temporary servers, control then scaled; identical text/reference/seeds 501-520, all 32 codebooks. No concurrent GPU test.','all_20_full_pcm_streams_exact':same,
            'control_ttfa':a['ttfa'],'scaled_ttfa':b['ttfa'],'control_fresh_single_ms':a['fresh_reference_to_pcm_ms'],'scaled_fresh_single_ms':b['fresh_reference_to_pcm_ms'],
            'control_cancellation_statuses':a['cancellation_recovery_statuses'],'scaled_cancellation_statuses':b['cancellation_recovery_statuses'],'temporary_servers_stopped':True}
    (RESULTS/f'http_scaled_dp4a_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
