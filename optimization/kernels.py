...
@triton.jit
def _fp8_gemv(X,W,S,Y,N:tl.constexpr,K:tl.constexpr,BK:tl.constexpr):
    row=tl.program_id(0)
    k=tl.arange(0,BK)
    x=tl.load(X+k,k<K,0).to(tl.float32)
    w=tl.load(W+row*K+k,k<K,0.0).to(tl.float32)
    scale=tl.load(S+row)
    tl.store(Y+row,tl.sum(w*x,0)*scale)


@triton.jit
def _tiled_weight_gemv(X,W,SCALE,Y,N:tl.constexpr,K:tl.constexpr,R:tl.constexpr,BK:tl.constexpr,QUANTIZED:tl.constexpr):
    rows=tl.program_id(0)*R+tl.arange(0,R)
    cols=tl.arange(0,BK)
    x=tl.load(X+cols,cols<K,0).to(tl.float32)
    w=tl.load(W+rows[:,None]*K+cols[None,:],(rows[:,None]<N)&(cols[None,:]<K),0.0).to(tl.float32)
    y=tl.sum(w*x[None,:],1)
    if QUANTIZED:
        y*=tl.load(SCALE+rows,rows<N,0)
    tl.store(Y+rows,y,rows<N)


@triton.jit
def _int4_gemv(X,W,SCALE,Y,K:tl.constexpr,GROUP:tl.constexpr,BK:tl.constexpr):
    row=tl.program_id(0)
    cols=tl.arange(0,BK)
    packed=tl.load(W+row*(K//2)+cols//2,cols<K,0).to(tl.int32)
    unsigned=(packed >> (4*(cols%2))) & 15
    signed=tl.where(unsigned>=8,unsigned-16,unsigned).to(tl.float32)
    scale=tl.load(SCALE+row*(K//GROUP)+cols//GROUP,cols<K,0)
    x=tl.load(X+cols,cols<K,0).to(tl.float32)
    tl.store(Y+row,tl.sum((signed*scale)*x,0))


@triton.jit
def _int4_pair_gemv(X,W,SCALE,Y,K:tl.constexpr,GROUP:tl.constexpr,BP:tl.constexpr):
    row=tl.program_id(0)
    pair=tl.arange(0,BP)
    packed=tl.load(W+row*(K//2)+pair,pair<K//2,0).to(tl.int32)
    low=packed & 15
    high=(packed >> 4) & 15
    low=tl.where(low>=8,low-16,low).to(tl.float32)
    high=tl.where(high>=8,high-16,high).to(tl.float32)
    scale=tl.load(SCALE+row*(K//GROUP)+(2*pair)//GROUP,pair<K//2,0)
    x0=tl.load(X+2*pair,pair<K//2,0).to(tl.float32)
    x1=tl.load(X+2*pair+1,pair<K//2,0).to(tl.float32)
    tl.store(Y+row,tl.sum((low*scale)*x0+(high*scale)*x1,0))


def quantized_linear_decode(x,weight,scale,fallback,tiled=False,fp8_prefill=False):
    """Weight-only GEMV; multi-token prefill uses BF16 weights."""
    if x.numel()!=x.shape[-1]:
        if fp8_prefill:
            if weight.dtype!=torch.float8_e4m3fn:raise ValueError('FP8 prefill requires FP8 weights')
            from .native_fp8 import fp8_prefill_linear
            return fp8_prefill_linear(x,weight,scale,'vllm')
        return torch.nn.functional.linear(x,fallback)
    n,k=fallback.shape
    y=torch.empty((*x.shape[:-1],n),dtype=x.dtype,device=x.device)
    if weight.dtype==torch.uint8:
        if tiled:
            _int4_pair_gemv[(n,)](x,weight,scale,y,k,k//scale.shape[1],triton.next_power_of_2(k//2),num_warps=4 if k<=4096 else 8)
        else:
            _int4_gemv[(n,)](x,weight,scale,y,k,k//scale.shape[1],triton.next_power_of_2(k),num_warps=4 if k<=4096 else 8)
    elif tiled:
        # H200 measurements rotate eight matrices, exceeding L2 capacity.
        rows=2 if weight.dtype==torch.int8 or k>4096 else 4
        warps=4 if weight.dtype==torch.int8 and n==24576 and k==4096 else 8
        _tiled_weight_gemv[(triton.cdiv(n,rows),)](x,weight,scale,y,n,k,rows,triton.next_power_of_2(k),True,num_warps=warps)
    else:
        _fp8_gemv[(n,)](x,weight,scale,y,n,k,triton.next_power_of_2(k),num_warps=4 if k<=4096 else 8)
    return y


# Compatibility with the initial MLP-only FP8 experiment.
fp8_linear_decode = quantized_linear_decode
...
def qk_rope_decode(qkv, q_weight, k_weight, cos, sin, cache, layer_idx, position, eps, backend=None, block=128, warps=4,context_capacity=None,quantize_output=False,native_attention=False,pdl=None):
    ...
    if quantize_output:
        if quantize_output==128:
            from .dp4a_group128 import reduce_attention
            return reduce_attention(partial,lse)
        if pdl:
