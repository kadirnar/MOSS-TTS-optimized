"""Compare ordinary/compressed FP32 scales with exact BF16 storage and explicit layouts."""
import argparse
import hashlib
import json
import statistics
...
def main():
    ...
    path=RESULTS/f'short_scales_paired_{a.tag}.json'
    ...
    swaps=[];allocations=[];names=('torch','short','compressed')
    for layer_index,layer in enumerate(model.language_model.layers):
        for module,key in ((layer.self_attn,'_quant_out_scale'),(layer.mlp,'_quant_down_scale')):
            ...
            for name in names[1:]:
                if name=='short':
                    values[name]=original.bfloat16()
                    meta={'logical_bytes':values[name].numel()*2,'allocated_bytes':values[name].numel()*2,'allocator':'Torch BF16'}
                else:values[name],meta=clone(original,compressed=True)
                assert torch.equal(values[name].float(),original)
                allocations.append({'layer':layer_index,'buffer':key,'variant':name,**meta})
            swaps.append((module,key,values))
    managers={}
    def select(name):
        for module,key,values in swaps:
            setattr(module,key,values[name])
            module._short_scale_projection=key[7:-6] if name=='short' else None
    ...
    result={'codebooks':32,'torch':torch.__version__,'rows':rows,'graph_rows':graph_rows,'checks':checks,
            ...
            'method':'Shared calibrated G32 model, BF16 prefill, FP32 codec, original head/context buckets. Only 72 out/down scale buffers vary: ordinary FP32, hardware-compressed FP32, or exact BF16 storage with explicit Gluon floating layouts. The Gluon dispatch flag is restored alongside each scale buffer. Separate graph sets retain every allocation owner. Eager buffers and graph dispatch both restored per variant. Rotating/reversed order and one excluded warmup triplet. Cached-voice complete streaming PCM; no reference encoding/network.',
            ...
            'short_vs_compressed_median_paired_gain_ms':statistics.median(r['records']['compressed']['ttfa_ms']-r['records']['short']['ttfa_ms'] for r in rows)}
    ...
    (RESULTS/f'short_scales_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    engine.codec.close()
