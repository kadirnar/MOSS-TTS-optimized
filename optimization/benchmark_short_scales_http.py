"""Sequential temporary loopback servers for the compressed FP32 versus exact BF16 scale comparison.
...
def main():
    ...
    base=[sys.executable,'-u','-m','optimization.server','--port','18084',
          ...
          '--decode-buckets','--attention-quant','--native-attention','--gateup-quant','--scaled-dp4a','--norm-projection','--audio-head-buckets']
    for mode in ('control','short'):
        name='http_short_scales_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--short-scales'] if mode=='short' else ['--compressed-scales'])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a'] and health['norm_projection'] and health['audio_head_buckets'] and health['short_scales']==(mode=='short') and health['compressed_scales']==(mode=='control')
    ...
    a=json.loads((RESULTS/f'http_short_scales_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_short_scales_short_{tag}.json').read_text())
    ...
    result={'method':'Sequential temporary servers, compressed-FP32 control then BF16 short scales; identical text/reference/seeds 501-520, all 32 codebooks. No concurrent GPU test.','all_20_full_pcm_streams_exact':same,
            'control_ttfa':a['ttfa'],'short_ttfa':b['ttfa'],'control_fresh_single_ms':a['fresh_reference_to_pcm_ms'],'short_fresh_single_ms':b['fresh_reference_to_pcm_ms'],
            'control_cancellation_statuses':a['cancellation_recovery_statuses'],'short_cancellation_statuses':b['cancellation_recovery_statuses'],'temporary_servers_stopped':True}
    (RESULTS/f'http_short_scales_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
