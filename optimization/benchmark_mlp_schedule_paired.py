"""Full-stream MLP schedule checks against the qualified eight-row down preset."""
import argparse
import hashlib
import json
import statistics
...
from .benchmark_mlp_schedule import configs
...
def main():
    ...
    p.add_argument('--binary-bundle',type=Path,default=RESULTS/'qkv_cluster_bundle_v6');p.add_argument('--configs',nargs='+',choices=tuple(configs()),default=['t_u1_d0','u_r32_ig1ir2'])
    ...
    path=RESULTS/f'mlp_schedule_paired_{a.tag}.json';assert not path.exists(),'Preserve evidence'
    ...
    choices={n:c for n,c in configs().items() if n=='control' or n in a.configs}
    ...
    def make_norm(tile):
        def call(*args,**kwargs):
            return original_norm(*args,**{**kwargs,**(tile if kwargs.get('fused',False) else {})})
        return call
    ...
    for name,choice in choices.items():
        ...
        down_dispatches[name]=make_down(choice.get('down',{}));norm_dispatches[name]=make_norm(choice.get('up',{}))
    ...
    if a.profile:
        ...
        prof.export_chrome_trace(str(RESULTS/f'mlp_schedule_profile_{a.tag}_trace.json'))
        (RESULTS/f'mlp_schedule_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    engine.codec.close()
