...
@dataclass
class AudioChunk:
    pcm: torch.Tensor  # CPU float32, mono, 24 kHz; safe after next graph replay
    frame: int
    elapsed_ms: float


class StreamingTTS:
    def __init__(self, model, codec, processor, *, graph=True, fused=True, codec_bf16=False, max_length=1024, greedy=False, fp8_mlp=False, weight_quantization='none', tiled_quantization=False, attention_backend='triton', attention_block=128, attention_warps=4, fused_residual=False, fused_gateup=False, fp8_prefill=False, codec_clock=False):
        self.model=model
        self.processor=processor
        self.llm=FastLLM(model,max_length=max_length,graph=graph,fused=fused,greedy=greedy,fp8_mlp=fp8_mlp,weight_quantization=weight_quantization,tiled_quantization=tiled_quantization,attention_backend=attention_backend,attention_block=attention_block,attention_warps=attention_warps,fused_residual=fused_residual,fused_gateup=fused_gateup,fp8_prefill=fp8_prefill)
        if codec_clock:
            if codec_bf16:raise ValueError('Shared codec clock is qualified for FP32 only')
            from .codec_clock import ClockedStreamingCodec
            self.codec=ClockedStreamingCodec(codec,graph=graph)
        else:self.codec=StreamingCodec(codec,graph=graph,bf16=codec_bf16)
        self.greedy=greedy
        self.last_metrics={}
        self._busy=False
    ...
    def stream(self,text,reference_codes=None,language="Chinese",max_new_tokens=400):
        if self._busy:
