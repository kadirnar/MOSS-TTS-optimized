"""Changed-input private graphs for exact CUDA register/resource gate/up checks."""
import argparse
import json
import torch
from .common import RESULTS
from .benchmark_gateup_resources import Chain, load_layer, poison, exact, make_up
...
def main():
    ...
    path = RESULTS/f'gateup_resources_validation_{args.tag}.json'
    if path.exists():
    ...
    control.up = make_dispatch(control.up)
    result = {'codebooks': 32, 'complete': False, 'bundle': args.bundle,
        'configs': args.configs, 'checks': [], 'edges': {},
        'scope': 'Selected exact one-CTA gate/up control. Private graphs, three real layers, changed real/zero/spike input and residual. '
