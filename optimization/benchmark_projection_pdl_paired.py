...
def main():
    ...
    enable(fast)
    modules=[m for layer in model.language_model.layers for m in (layer.self_attn,layer.mlp)]
    def select(name):
        config=None if name=='control' else {'norm_trigger':1,'projection_trigger':3,'scale_prefetch':name=='prefetch'}
        fast.projection_pdl=config
        for module in modules:module._projection_pdl=config
    ...
    result={'codebooks':32,'torch':torch.__version__,'rows':rows,'graph_rows':graph_rows,'checks':checks,
            'all_pcm_exact':True,'integration':'Per-model and per-projection attributes; no global function replacement.',
