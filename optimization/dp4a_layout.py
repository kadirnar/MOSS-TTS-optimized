"""Experimental explicit integer and floating layouts with scaled INT4 dots.

Only integer sums cross layouts. Scales are loaded directly in the fixed
floating reduction layout, avoiding the activation-scale redistribution.
"""
import torch
import triton
...
def linear(x,w,s,*,fused=False,rows=4,warps=4,integer_groups=1,integer_rows=1,prequantized):
    ...
    _kernel[(triton.cdiv(n,rows),)](q,sx,w,s,y,oq,os,n,k,triton.next_power_of_2(k//32),rows,fused,warps,integer_groups,integer_rows,num_warps=warps)
