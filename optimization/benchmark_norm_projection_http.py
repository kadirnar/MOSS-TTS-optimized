"""Sequential temporary loopback servers for the norm/projection fusion comparison.
...
def main():
    ...
    for mode in ('control','fused'):
        name='http_norm_projection_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--norm-projection'] if mode=='fused' else [])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a'] and health['norm_projection']==(mode=='fused')
    ...
    a=json.loads((RESULTS/f'http_norm_projection_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_norm_projection_fused_{tag}.json').read_text())
    ...
    (RESULTS/f'http_norm_projection_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
