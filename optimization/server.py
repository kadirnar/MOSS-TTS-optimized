"""Local PCM streaming service with cached voice references and cancellation.

Run: /venv/main/bin/python -m optimization.server --port 18080
"""
import argparse
import asyncio
import base64
...
def create_app(*, attention_block=128, fused_residual=False, fused_gateup=False, weight_quantization='none',calibration=None,packing_plan=None,decode_buckets=False,metrics_file=None,attention_quant=False,native_attention=False,gateup_quant=False,scaled_dp4a=False,cuda_only_seed=False,norm_projection=False,audio_head_buckets=False,compressed_scales=False,short_scales=False,projection_pdl=False,attention_pdl=False,codec_clock=False,first_audio_graph=False,bulk_prefetch=False,prefill_qkv=False,prefill_pointwise=False,async_output=False,output_weight_prefetch=False,qkv_cluster=False,down_tile8=False,gateup_compiler=False,attention_history=False):
    ...
    if attention_history and not gateup_compiler:raise ValueError('Historical attention preloads require the selected gate/up compiler preset')
    if gateup_compiler and not down_tile8:raise ValueError('Gate/up compiler option requires the eight-row down preset')
    ...
    def initialize():
        ...
        if gateup_compiler:
            ...
            state['gateup_compiler']=enable(engine.llm)
        if attention_history:
            from .attention_history_model import enable
            state['attention_history']=enable(engine.llm)
    ...
    @app.get('/health')
    def health():
        return {'ready':'engine' in state,'model':'OpenMOSS-Team/MOSS-TTS-v1.5',
            ...
            'qkv_cluster':qkv_cluster,'down_tile8':down_tile8,'gateup_compiler':gateup_compiler,'attention_history':attention_history,
...
def main():
    ...
    parser.add_argument('--attention-history',action='store_true',help='Preload historical KV entries before the attention dependency wait')
