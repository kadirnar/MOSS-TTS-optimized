"""Sequential temporary loopback servers for the exact BF16 scales versus projection PDL comparison.
...
def main():
    ...
    base=[sys.executable,'-u','-m','optimization.server','--port','18084',
          ...
          '--decode-buckets','--attention-quant','--native-attention','--gateup-quant','--scaled-dp4a','--norm-projection','--audio-head-buckets','--short-scales']
    for mode in ('control','pdl'):
        name='http_projection_pdl_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--projection-pdl'] if mode=='pdl' else [])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a'] and health['norm_projection'] and health['audio_head_buckets'] and health['short_scales'] and not health['compressed_scales'] and health['projection_pdl']==(mode=='pdl')
    ...
    a=json.loads((RESULTS/f'http_projection_pdl_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_projection_pdl_pdl_{tag}.json').read_text())
    ...
    result={'method':'Sequential temporary servers, exact BF16 scales control then projection PDL with scale preloads; identical text/reference/seeds 501-520, all 32 codebooks. No concurrent GPU test.','all_20_full_pcm_streams_exact':same,
    ...
    (RESULTS/f'http_projection_pdl_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
