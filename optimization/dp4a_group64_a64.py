"""Experimental G64 weights with G32/G64 activations and SM90 PDL.
...
@gluon.jit
def _finish(sums,xscale,S,base,N:gl.constexpr,K:gl.constexpr,BG:gl.constexpr,R:gl.constexpr,F:gl.constexpr,FACTOR:gl.constexpr,AG:gl.constexpr):
    if FACTOR and AG==32:
        ...
        scales=gl.load(S+rows[:,None]*(K//64)+(groups//(64//AG))[None,:],(rows[:,None]<N)&(groups[None,:]<K//AG),0).to(gl.float32)
        return gl.sum(sums*(scales*xscale[None,:]),1)


@gluon.jit
def _projection(a,b,xscale,W,S,base,N:gl.constexpr,R:gl.constexpr,I:gl.constexpr,F:gl.constexpr,FACTOR:gl.constexpr,AG:gl.constexpr):
    rows=base+gl.arange(0,R,layout=gl.SliceLayout(1,gl.SliceLayout(2,I)))
    groups=gl.arange(0,4096//AG,layout=gl.SliceLayout(0,gl.SliceLayout(2,I)))
    c=gl.arange(0,AG//8,layout=gl.SliceLayout(0,gl.SliceLayout(0,I)))
    pos=groups[None,:,None]*(AG//8)+gl.expand_dims(gl.expand_dims(c,0),0)
    ...
    return _finish(sums,xscale,S,base,N,4096,4096//AG,R,F,FACTOR,AG)
...
def _norm_kernel(X,RES,NW,W,S,SUM,Y,OQ,OS,NY,NQ,NS,N:gl.constexpr,R:gl.constexpr,
            ...
            DEBUG:gl.constexpr,PDL:gl.constexpr,TRIGGER:gl.constexpr,FACTOR:gl.constexpr,DIV:gl.constexpr,AG:gl.constexpr,OAG:gl.constexpr):
    ...
    value=_projection(a,b,scale,W,S,base,N*2 if FUSED else N,R,I,F,FACTOR,AG).to(gl.bfloat16)
    if FUSED:
        up=_projection(a,b,scale,W,S,base+N,N*2,R,I,F,FACTOR,AG).to(gl.bfloat16).to(gl.float32)
    ...
    if FUSED:
        output=gl.reshape(value.to(gl.float32),(R//OAG,OAG))
        ...
        groups=gl.program_id(0)*(R//OAG)+gl.arange(0,R//OAG,layout=output_scale.type.layout)
        gl.store(OS+groups,output_scale,groups<N//OAG)
...
def _plain_kernel(Q,XS,W,S,Y,N:gl.constexpr,K:gl.constexpr,BG:gl.constexpr,R:gl.constexpr,
                  ...
                  PDL:gl.constexpr,TRIGGER:gl.constexpr,DIV:gl.constexpr,AG:gl.constexpr):
    ...
    scales=gl.load(S+rf[:,None]*(K//64)+(gf//(64//AG))[None,:],(rf[:,None]<N)&(gf[None,:]<K//AG),0).to(gl.float32)
    if PDL:wait()
    if PDL and TRIGGER==1:trigger()
    ...
    sums=gl.convert_layout((gl.sum(dot,2)>>4).to(gl.float32),F);xs=gl.load(XS+gf,gf<K//AG,0)
    if FACTOR and AG==32:
...
def norm_linear(x,residual,norm_weight,eps,w,s,*,fused=False,rows=8,integer_groups=1,integer_rows=1,
                debug=False,pdl=True,trigger_mode=1,factor=0,divisor=16,activation_group=64,output_group=32,return_kernel=False):
    _validate_weights(x,w,s,4096);n2=w.shape[0];n=n2//2 if fused else n2
    ag=activation_group;oag=output_group
    if ag not in (32,64) or oag not in (32,64) or fused and (rows%oag or n%oag):raise ValueError("Complete activation groups required")
    ...
    oq=torch.empty(n if fused else 0,device=x.device,dtype=torch.int8);os=torch.empty(n//oag if fused else 0,device=x.device)
    ...
    compiled=_norm_kernel[(triton.cdiv(n,rows),)](x,residual,norm_weight,w,s,summed,y,oq,os,ny,nq,ns,n,rows,eps,residual is not None,fused,integer_groups,integer_rows,debug,pdl,trigger_mode,factor,divisor,ag,oag,num_warps=4,launch_pdl=pdl)
...
def linear(x,w,s,*,prequantized,rows=4,warps=4,integer_groups=1,integer_rows=1,
           pdl=True,trigger_mode=3,factor=0,divisor=0,activation_group=64,return_kernel=False):
    k=x.numel();_validate_weights(x,w,s,k);n=w.shape[0];q,sx=prequantized;ag=activation_group
    if ag not in (32,64):raise ValueError("G32/G64 activations required")
    ...
    if q.dtype!=torch.int8 or q.numel()!=k or sx.dtype!=torch.float32 or sx.numel()!=k//ag:raise ValueError('Matching grouped activation buffers required')
    ...
    y=torch.empty((*x.shape[:-1],n),device=x.device,dtype=x.dtype)
    compiled=_plain_kernel[(triton.cdiv(n,rows),)](q,sx,w,s,y,n,k,triton.next_power_of_2(k//ag),rows,warps,integer_groups,integer_rows,factor,pdl,trigger_mode,divisor,ag,num_warps=warps,launch_pdl=pdl)
