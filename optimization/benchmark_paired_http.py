...
def main():
    ...
    command = [sys.executable, '-u', '-m', 'optimization.paired_http_server', '--port', '18084',
        ...
        '--bulk-prefetch', '--prefill-qkv', '--prefill-pointwise', '--output-weight-prefetch', '--qkv-cluster']
    result = {'codebooks': 32, 'complete': False, 'cached_rows': [], 'fresh_rows': [],
        'method': 'Single temporary loopback process, one shared model and FP32 codec, two separately captured graph sets. '
                  'Selected clustered G32 control versus qualified eight-row down tile. '
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    pass
                time.sleep(.5)
            assert health['benchmark_only'] and health['variants'] == ['control', 'down8'] and health['codebooks'] == 32
            ...
            for workload, rounds, seed_base in (('cached', args.rounds, 501), ('fresh', args.fresh_rounds, 1501)):
                for repeat in range(-1, rounds):
                    order = ['control', 'down8'] if repeat % 2 == 0 else ['down8', 'control']
                    records = {}
                    ...
                    assert records['control']['pcm_sha256'] == records['down8']['pcm_sha256']
                    row = {'round': repeat, 'seed': seed_base+repeat, 'order': order,
                        'records': records, 'gain_ms': records['control']['ttfa_ms']-records['down8']['ttfa_ms']}
                    if repeat >= 0:
            ...
            assert session.post(url+'/v1/audio/speech', json={'input':'Hello','voice':'missing','variant':'down8'}, timeout=5).status_code == 404
            ...
            while time.monotonic()-started < 3:
                response = session.post(url+'/v1/audio/speech', json={'input':'取消之后继续生成。',
                    'voice':voice,'max_new_tokens':96,'variant':'down8','benchmark_id':'recovery'}, stream=True, timeout=30)
    ...
    for workload in ('cached', 'fresh'):
        ...
        for row in rows:
            ...
            assert row['records']['control']['rng_sha256'] == row['records']['down8']['rng_sha256']
        result[workload+'_summary'] = {'ttfa': {n:stats([r['records'][n]['ttfa_ms'] for r in rows]) for n in ('control','down8')},
