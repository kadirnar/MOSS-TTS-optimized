...
def main():
    ...
    command = [sys.executable, '-u', '-m', 'optimization.paired_history_http_server', '--port', '18084',
    ...
    result = {'codebooks': 32, 'complete': False, 'cached_rows': [], 'fresh_rows': [],
        'method': 'Single temporary loopback process, one shared model and FP32 codec, two separately captured graph sets. '
                  'Selected eight-row/exact gate-up G32 control versus historical KV preloads and early QKV/attention release. '
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    pass
                time.sleep(.5)
            assert health['benchmark_only'] and health['variants'] == ['control', 'history'] and health['codebooks'] == 32
            ...
            for workload, rounds, seed_base in (('cached', args.rounds, 501), ('fresh', args.fresh_rounds, 1501)):
                for repeat in range(-1, rounds):
                    order = ['control', 'history'] if repeat % 2 == 0 else ['history', 'control']
                    records = {}
                    ...
                    assert records['control']['pcm_sha256'] == records['history']['pcm_sha256']
                    row = {'round': repeat, 'seed': seed_base+repeat, 'order': order,
                        'records': records, 'gain_ms': records['control']['ttfa_ms']-records['history']['ttfa_ms']}
                    if repeat >= 0:
            ...
            assert session.post(url+'/v1/audio/speech', json={'input':'Hello','voice':'missing','variant':'history'}, timeout=5).status_code == 404
            ...
            while time.monotonic()-started < 3:
                response = session.post(url+'/v1/audio/speech', json={'input':'取消之后继续生成。',
                    'voice':voice,'max_new_tokens':96,'variant':'history','benchmark_id':'recovery'}, stream=True, timeout=30)
    ...
    for workload in ('cached', 'fresh'):
        ...
        for row in rows:
            ...
            assert row['records']['control']['rng_sha256'] == row['records']['history']['rng_sha256']
        result[workload+'_summary'] = {'ttfa': {n:stats([r['records'][n]['ttfa_ms'] for r in rows]) for n in ('control','history')},
    ...
    prior = json.loads((RESULTS/'paired_http_gateup_v1.json').read_text())
    assert all(row['records']['control']['pcm_sha256'] == old['records']['gateup38']['pcm_sha256']
               for row,old in zip(result['cached_rows'], prior['cached_rows']))
    result['prior_cached_pcm_exact'] = min(len(result['cached_rows']),len(prior['cached_rows']))
