...
def linear(x,packed,scales,*,rows=4,warps=4,mode=2,paired=False,fused=False,prequantized=None,scale_mode=0):
    ...
    _gemv[(triton.cdiv(out_n,rows),)](q,sx,packed,scales,out,oq,os,out_n,k,triton.next_power_of_2(k//32),rows,mode,paired,fused,scale_mode,num_warps=warps)
    return (out,(oq,os)) if fused else out


def enable_scaled(llm):
    """Install the exact tested configurations before any graph capture."""
    if llm.graph is not None or llm.prefill_graphs:
        raise RuntimeError('Enable scaled DP4A unpacking before graph capture')
    modules=[m for layer in llm.model.language_model.layers for m in (layer.self_attn,layer.mlp)]
    shapes={'qkv':(6144,2048),'out':(4096,2048),'up':(24576,2048),'down':(4096,6144)}
    for m in modules:
        names=('qkv','out') if hasattr(m,'_qkv') else ('up','down')
        plan=getattr(m,'_dp4a_packing',{})
        if not getattr(m,'_dp4a_grouped_activation',False) or getattr(m,'_dp4a_group',None)!=32:
            raise ValueError('Scaled unpacking requires grouped G32 DP4A activations')
        if 'up' in names and not getattr(m,'_fused_gateup_quant',False):
            raise ValueError('Enable the exact gate/up quantizer fusion first')
        for name in names:
            if plan.get(name,{}).get('scheme')!='interleaved' or tuple(getattr(m,'_quant_'+name).shape)!=shapes[name]:
                raise ValueError('Scaled unpacking requires the MOSS-TTS 8B interleaved weight plan')
            expected_dtype=torch.bfloat16 if name in ('qkv','up') else torch.float32
            if getattr(m,'_quant_'+name+'_scale').dtype!=expected_dtype:
                raise ValueError('Scaled unpacking requires the tested BF16 QKV/up and FP32 out/down scales')
    for m in modules:
        names=('qkv','out') if hasattr(m,'_qkv') else ('up','down')
        m._scaled_dp4a={name:dict(SELECTED[name]) for name in names}
