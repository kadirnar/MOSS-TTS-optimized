"""Full-stream historical KV preload checks against the selected exact preset."""
import argparse
import hashlib
import json
import statistics
...
from .attention_history_model import Hidden
from .attention_history import configs as history_configs
...
def main():
    ...
    p.add_argument('--binary-bundle',type=Path,default=RESULTS/'qkv_cluster_bundle_v6');p.add_argument('--configs',nargs='+',required=True)
    ...
    path=RESULTS/f'attention_history_paired_{a.tag}.json';assert not path.exists(),'Preserve evidence'
    ...
    selected.update(early_options);selected_launchers.update(early_launchers)
    choices={'control':None,**{n:history_configs()[n] for n in a.configs}}
    from .gateup_compiler import make_dispatch
    original_norm=make_dispatch(fast._bulk_norm_linear)
    ...
    for name,choice in choices.items():
        ...
        key='c8_exact' if choice is not None and choice['producer']==1 else 'c8_t2_exact'
        dispatches[name]=Hidden(fast,selected_launchers[key],selected[key],choice)
        down_dispatches[name]=make_down({});norm_dispatches[name]=original_norm
    ...
    result={'codebooks':32,'checks':checks,'rows':rows,'complete':False,
        'method':'Qualified G32/FP32-codec model with selected gate/up, clustered QKV, register-preloaded output and eight-row down projection. Only historical KV loads and attention/producer scheduling differ. Current-token reads retain the grid dependency wait. All 32 codebooks retained. Independent context/head/first-audio graphs and eager dispatch. Rotating/reversing cached-voice complete requests, one excluded warmup. Full PCM and final RNG compared; timing excludes network and reference registration.',
        'binary_bundle':str(a.binary_bundle),'early_producer_bundle':str(RESULTS/'qkv_cluster_bundle_history_v1'),
        'configs':choices}
    ...
    if a.profile:
        ...
        prof.export_chrome_trace(str(RESULTS/f'attention_history_profile_{a.tag}_trace.json'))
        (RESULTS/f'attention_history_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    engine.codec.close()
