...
def main():
    ...
    path=RESULTS/f'attention_pdl_paired_{a.tag}.json'
    ...
    names=('control','late','preload');managers={}
    from .projection_pdl import enable as enable_projection
    enable_projection(fast)
    from .attention_pdl import enable
    enable(fast)
    modules=[layer.self_attn for layer in model.language_model.layers]
    def select(name):
        config=None if name=='control' else ({'pdl':True,'qk':1,'attention':2,'reduce':1,'preload':False} if name=='late' else {'pdl':True,'qk':2,'attention':1,'reduce':1,'preload':True})
        fast.attention_pdl=config
    ...
    result={'codebooks':32,'torch':torch.__version__,'rows':rows,'graph_rows':graph_rows,'checks':checks,
            'all_pcm_exact':True,'integration':'Per-model/per-attention attributes; projection PDL enabled for every variant.',
            'method':'One shared G32 model, short scales and qualified projection PDL, BF16 prefill, FP32 codec, context/head buckets. Control uses ordinary QK/attention/reduction. Late candidate hints after QK wait, after attention logits, after reduction wait. Preload candidate reads immutable Q/K norm weights before waiting, hints after QK normalization, after attention/reduction waits. Separate graph sets, eager dispatch restored. Rotating/reversed order; one excluded warmup triplet. Cached cloned voice and all 32 codebooks; complete float32 PCM, no registration/network.',
    ...
    prof.export_chrome_trace(str(RESULTS/f'attention_pdl_profile_{a.tag}_trace.json'))
    (RESULTS/f'attention_pdl_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    select('control')
    engine.codec.close()
