...
from .dp4a_packing import pack_interleaved,unpack_interleaved
from .dp4a_scaled import SELECTED,linear as reference
from .dp4a_mma import linear,library,pack_mma,unpack_mma
...
def main():
    ...
    outfile=RESULTS/f'dp4a_mma_kernels_{args.tag}.json';assert not outfile.exists(),'Preserve old results'
    ...
    for name in ('qkv','out','up','down'):
        ...
        def candidate(x,w,s,qx,c):return linear(x,prepared[w.data_ptr()],s,prequantized=qx,fused=name=='up',**c)
        ...
        prepared={weight.data_ptr():pack_mma(weight,fused=name=='up') for weight,_ in ring}
        assert torch.equal(unpack_mma(prepared[w.data_ptr()],fused=name=='up'),unpack_interleaved(w))
        actual=aa=bb=None
