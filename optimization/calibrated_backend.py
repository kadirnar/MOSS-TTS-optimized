...
def install_calibrated(llm,folder,backend='marlin'):
    folder=Path(folder)
    config=json.loads((folder/'config.json').read_text())
    if config['tts_revision']!=TTS_REVISION:raise ValueError('Calibrated checkpoint revision mismatch')
    if backend not in ('marlin','dp4a'):raise ValueError('Unsupported calibrated backend')
    ...
    for i,layer in enumerate(llm.model.language_model.layers):
        ...
        for module,name,weight in targets:
            ...
            if backend=='marlin':
                ...
                op=MarlinLinear(weight,4,config['group'],signed=unpack_signed(packed),scales=scales)
                op.release_reference()
                setattr(module,'_marlin_'+name,op)
