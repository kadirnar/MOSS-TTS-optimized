...
template<int K,bool SHORT,bool CACHE> __device__ __forceinline__ float projection(
    ...
    for(int j=0;j<C;++j){
        ...
        int dot=dot32(w+row*(K/8)+g*4,a,valid&&valid_row);
        float sx;if constexpr(CACHE)sx=xsc[j];else sx=valid?__ldg(xs+g):0.f;
        sums[j]=(float)dot;scales[j]=__fmul_rn(load_scale<SHORT>(s,row*G+g,valid&&valid_row),sx);
    }
...
template<int K,bool SHORT,bool FUSED,int WARPS,int REPEAT,bool CACHE>
__global__ void native_projection(const int8_t* __restrict__ q,const float* __restrict__ xs,
    ...
    for(int it=0;it<COUNT;++it){
        ...
        float value=projection<K,SHORT,CACHE>(q,xs,w,s,row,row<n,cache,xsc,part);
        if constexpr(FUSED){
            float up=projection<K,SHORT,CACHE>(q,xs,w,s,row+n,row<n,cache,xsc,part);
...
int invoke(void* q,void* xs,void* w,void* s,void* y,void* oq,void* os,int n,void* stream){
    ...
    native_projection<K,SHORT,FUSED,WARPS,REPEAT,CACHE><<<(n+ROWS-1)/ROWS,WARPS*32,0,(cudaStream_t)stream>>>(
        (int8_t*)q,(float*)xs,(uint32_t*)w,s,(__nv_bfloat16*)y,(int8_t*)oq,(float*)os,n);
    return (int)cudaGetLastError();
}
#define RUN(W,R) if(warps==W&&repeat==R){if(cache)return invoke<K,SHORT,FUSED,W,R,true>(q,xs,w,s,y,oq,os,n,stream);else return invoke<K,SHORT,FUSED,W,R,false>(q,xs,w,s,y,oq,os,n,stream);}
...
extern "C" int launch_dp4a_native(void* q,void* xs,void* w,void* s,void* y,void* oq,void* os,
    ...
    if(fused&&k==4096&&short_scale)return choose<4096,true,true>(q,xs,w,s,y,oq,os,n,warps,repeat,cache,stream);
    if(!fused&&k==4096&&short_scale)return choose<4096,true,false>(q,xs,w,s,y,oq,os,n,warps,repeat,cache,stream);
    if(!fused&&k==4096&&!short_scale)return choose<4096,false,false>(q,xs,w,s,y,oq,os,n,warps,repeat,cache,stream);
    if(!fused&&k==12288&&!short_scale)return choose<12288,false,false>(q,xs,w,s,y,oq,os,n,warps,repeat,cache,stream);
    return (int)cudaErrorInvalidValue;
}
