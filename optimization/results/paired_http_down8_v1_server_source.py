...
Generated from server.py SHA256 d040811ca58c6a1affec0ebe6323ec73aab86ed43b88200e64e8fb60cbff3084.
...
class SpeechRequest(BaseModel):
    ...
    variant:Literal['control','down8']='control'
...
def setup_variants(engine, state):
    from .down_tile import make_dispatch
    ...
    original=[m._bulk_down_linear for m in modules]
    variants={'control':(fast._audio_head_buckets,engine._first_audio_graphs,original)}
    fn=make_dispatch()
    for m in modules:m._bulk_down_linear=fn
    fast._audio_head_buckets=None;fast.step=types.MethodType(FastLLM.step,fast)
    fast.warmup();manager=capture(fast);manager.install()
    engine._first_audio_graphs=None;enable(engine)
    variants['down8']=(manager,engine._first_audio_graphs,[fn]*len(modules))
    def select(name):
        manager,bursts,functions=variants[name]
        fast._audio_head_buckets=manager;fast.step=manager.step;engine._first_audio_graphs=bursts
        for module,fn in zip(modules,functions,strict=True):module._bulk_down_linear=fn
...
def create_app(*, attention_block=128, fused_residual=False, fused_gateup=False, weight_quantization='none',calibration=None,packing_plan=None,decode_buckets=False,metrics_file=None,attention_quant=False,native_attention=False,gateup_quant=False,scaled_dp4a=False,cuda_only_seed=False,norm_projection=False,audio_head_buckets=False,compressed_scales=False,short_scales=False,projection_pdl=False,attention_pdl=False,codec_clock=False,first_audio_graph=False,bulk_prefetch=False,prefill_qkv=False,prefill_pointwise=False,async_output=False,output_weight_prefetch=False,qkv_cluster=False,down_tile8=False):
    if down_tile8 or not (qkv_cluster and audio_head_buckets and first_audio_graph and metrics_file):
        raise ValueError('Benchmark requires clustered control, audio-head/initial-audio graphs and metrics; omit down-tile8')
    ...
    @app.get('/health')
    def health():
        return {'benchmark_only':True,'variants':['control','down8'],'ready':'engine' in state,'model':'OpenMOSS-Team/MOSS-TTS-v1.5',
