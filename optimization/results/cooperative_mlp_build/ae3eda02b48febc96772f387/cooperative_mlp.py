...
def source(up_ptx, up_signature, down_ptx, down_signature, blocks, early):
...
.reqntid 128
{{
 .reg .u32 %virtual, %block, %bar_tid, %step, %old, %seen, %flip;
 .reg .u64 %counter;
 .reg .pred %again, %leader, %blockzero, %pending;
...
UP_LOOP:
 ...
 selp.u32 %step, {0x80000000-(blocks-1)}, 1, %blockzero;
...
class Fused:
    def __init__(self, entry, down_kernel, *, blocks=384, early=False, cap=None):
        if blocks not in (128, 256, 384) or cap not in (None, 128, 144, 160, 168):
        ...
        ptx = source(up_ptx, record['pointer_signature'], down_kernel.asm['ptx'], down_signature, blocks, early)
        if cap is not None:
    ...
    def __call__(self, entry, index=0):
        ...
        attributes[0].id = 2; attributes[0].value.serialization = 1  # COOPERATIVE
        attributes[1].id = 6; attributes[1].value.serialization = 1  # PDL
        cfg = LaunchConfig(self.blocks, 1, 1, 128, 1, 1, self.shared,
            torch.cuda.current_stream(x.device).cuda_stream, attributes, 2)
