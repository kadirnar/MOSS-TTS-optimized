"""Complete exact PDL/control streaming comparison with shared weights and caches."""
import argparse
import hashlib
import json
import statistics
...
from . import dp4a_norm_projection as norm_module
...
from .dp4a_layout_pdl_prefetch import linear as projection_prefetch
...
def main():
    ...
    path=RESULTS/f'projection_pdl_paired_{a.tag}.json'
    ...
    short_module.enable(fast)
    names=('control','pdl','prefetch');managers={}
    original_norm=norm_module.linear;original_projection=short_module.linear
    def select(name):
        norm_module.linear=original_norm if name=='control' else partial(norm_pdl,trigger_mode=1)
        short_module.linear=original_projection if name=='control' else partial(projection_pdl,trigger_mode=3)
        if name=='prefetch':short_module.linear=partial(projection_prefetch,trigger_mode=3,prefetch=1)
        if name in managers:fast.step=managers[name].step
    engine.warmup((128,160,256,512));managers['control']=capture(fast)
    for name in names[1:]:
        select(name);fast.warmup();managers[name]=capture(fast)
    fixture=torch.load(RESULTS/'fixture.pt',weights_only=True)
    ids=fixture['inputs']['input_ids'].cuda();select('control')
    ...
    for repeat in range(-1,a.rounds):
        ...
        assert all(r['pcm_float32_sha256']==records['control']['pcm_float32_sha256'] for r in records.values())
        gains={name:records['control']['ttfa_ms']-records[name]['ttfa_ms'] for name in names[1:]}
    ...
    result={'codebooks':32,'torch':torch.__version__,'rows':rows,'graph_rows':graph_rows,'checks':checks,
            'all_pcm_exact':True,
            'method':'One shared G32 model with exact BF16 short scales, BF16 prefill, FP32 codec and context/head buckets. PDL waits at projection start; QKV/up trigger after wait, out/down trigger before stores. Third variant preloads only out/down static weight scales before waiting. Separate graph sets; eager Python dispatch restored with graph selection. Rotating/reversed order, one excluded warmup triplet. Cached cloned voice, all 32 codebooks, complete float32 PCM; no registration/network.',
    ...
    (RESULTS/f'projection_pdl_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    norm_module.linear=original_norm;short_module.linear=original_projection
    engine.codec.close()
