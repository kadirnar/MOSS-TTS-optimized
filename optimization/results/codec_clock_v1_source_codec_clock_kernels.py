...
def attention(projected,cos_sin,state,heads,clock,*,first=False,kv_only=False):
    ...
    if not (first and t==1):
        _attention[(heads,t)](q,k,v,clock,out,t,heads,capacity,triton.next_power_of_2(t if first else capacity),first,num_warps=8)
    return out
