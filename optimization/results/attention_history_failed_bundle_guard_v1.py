...
def chains(choices):
    options, launchers = load_bundle(RESULTS / 'qkv_cluster_bundle_v4')
    selected = json.loads((RESULTS / 'qkv_cluster_bundle_v6/manifest.json').read_text())
    earlier = json.loads((RESULTS / 'qkv_cluster_bundle_v4/manifest.json').read_text())
    assert all(earlier['binaries'][n]['cubin_sha256'] == r['cubin_sha256'] for n, r in selected['binaries'].items())
    result = {}
