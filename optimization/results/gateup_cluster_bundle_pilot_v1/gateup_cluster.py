...
def _kernel(X, RES, NW, W, S, SUM, Y, OQ, OS, NY, NQ, NS,
            ...
            DIV: gl.constexpr, TRIGGER: gl.constexpr, DEBUG: gl.constexpr,
    ...
    if DEBUG:
        ...
        gl.store(NQ+i, gl.reshape(quant, (4096,)).to(gl.int8), pid == 0)
    ...
    gl.store(OQ+rows, gl.reshape(output_quant, (32,)).to(gl.int8))
...
def linear(x, residual, norm_weight, eps, w, s, *, ctas=2,
           ...
           debug=False, return_kernel=False, compiled=None):
    ...
    if compiled is None:
        kernel = _kernel[(384,)](x, residual, norm_weight, w, s, summed, y, oq, os, ny, nq, ns,
            eps, residual is not None, ctas, divisor, trigger_mode, debug,
