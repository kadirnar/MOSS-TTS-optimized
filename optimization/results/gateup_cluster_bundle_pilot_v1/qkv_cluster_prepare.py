...
@gluon.jit
def _sub(x,y):
    return gl.inline_asm_elementwise('sub.rn.f32 $0,$1,$2;','=f,f,f',[x,y],dtype=gl.float32,is_pure=True,pack=1)
...
@gluon.jit
def _head_store(head,h,p,QW,KW,COS,SIN,Q,KC,L:gl.constexpr,HEPS:gl.constexpr,TRIGGER:gl.constexpr):
    O:gl.constexpr=head.type.layout
    ...
    # The selected decode PTX uses 0-xs (including its signed-zero behavior).
    # A sign-bit flip is numerically equivalent on nonzero inputs, but differs
    # on zero-valued heads and would change the exact KV-cache contract.
    signed=gl.where(d<64,_sub(0.0,swapped),swapped)
...
def _kernel(X,RES,NW,W,S,SUM,Y,QW,KW,COS,SIN,Q,KC,VC,POS,
            ...
            TRIGGER:gl.constexpr,DEBUG:gl.constexpr,LEGACY:gl.constexpr,LEGACY_NORM:gl.constexpr,LEADER:gl.constexpr,
    ...
    if h<40:
        ...
        if not LEADER or rank==0:
            _head_store(head,h,p,QW,KW,COS,SIN,Q,KC,L,HEPS,TRIGGER)
    else:
        if TRIGGER==3:trigger()
        rows=gl.arange(0,128,layout=value.type.layout)
...
def launch(x,residual,norm_weight,eps,w,s,q_weight,k_weight,cos,sin,kc,vc,position,head_eps,
           *,ctas=4,integer_groups=1,integer_rows=1,divisor=0,trigger_mode=1,legacy_projection=False,legacy_norm=False,leader_only=False,debug=False,return_kernel=False,compiled=None):
    if torch.cuda.get_device_capability()!=(9,0):raise ValueError('SM90 experiment required')
    if ctas not in (1,2,4,8) or integer_groups not in (1,2,4) or integer_rows not in (1,2,4) or divisor not in (0,8,16,32) or trigger_mode not in (0,1,2,3):
    ...
    if any(t.data_ptr()%16 for t in tensors):raise ValueError('Sixteen-byte aligned CUDA buffers required')
    ...
    if compiled is None:
        kernel=_kernel[(48,)](x,residual,norm_weight,w,s,summed,projected,q_weight,k_weight,cos,sin,q,kc,vc,position,
            eps,head_eps,kc.shape[-2],residual is not None,ctas,integer_groups,integer_rows,divisor,trigger_mode,debug,legacy_projection,legacy_norm,leader_only,
