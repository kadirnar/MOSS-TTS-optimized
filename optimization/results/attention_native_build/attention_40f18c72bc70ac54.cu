...
__global__ void attention_ordered(const __nv_bfloat16* __restrict__ q,
    ...
    #pragma unroll
    for(int j=0;j<8;++j)qq[j]=__bfloat162float(q[head*128+d+j]);
    #pragma unroll
    for(int r=0;r<4;++r){
        int t=split*32+warp*2+lane/16+r*8;
        float kk[8];
        #pragma unroll
        for(int j=0;j<8;++j)kk[j]=t<=p?__bfloat162float(k[((head/4)*length+t)*128+d+j]):0;
    ...
    for(int j=0;j<8;++j){
        ...
        for(int r=0;r<4;++r){
            int t=split*32+warp*2+lane/16+r*8;
            vv[r]=t<=p?__bfloat162float(v[((head/4)*length+t)*128+d+j]):0;
        }
        float z=mul(prob[1],vv[1]);
        z=__fmaf_rn(prob[0],vv[0],z);
        z=__fmaf_rn(prob[2],vv[2],z);
        z=__fmaf_rn(prob[3],vv[3],z);
