...
__global__ void mma_projection(const int8_t* __restrict__ q,const float* __restrict__ xs,
    ...
    for(int tile=warp;tile<(FUSED?ROWS:ROWS/2);tile+=WARPS){
        ...
        for(int base=0;base<BG;base+=8){
            ...
            if(g<G&&row<n){
                int tile_row=FUSED?row:row/2;
                packed=__ldg((const uint2*)(w+tile_row*(G/8)*64+(base/8)*64+lane*2));
            }
            uint32_t a0=(packed.x<<4)&0xf0f0f0f0u,a1=packed.x&0xf0f0f0f0u;
            uint32_t a2=(packed.y<<4)&0xf0f0f0f0u,a3=packed.y&0xf0f0f0f0u;
