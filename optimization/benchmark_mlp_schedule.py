"""Producer release and MLP layouts with the qualified eight-row down projection."""
import argparse
import hashlib
import json
import statistics
...
def configs():
    ...
    for up in (0, 1, 2, 3):
        for down in (0, 1, 3):
            choices[f't_u{up}_d{down}'] = {'up': {'trigger_mode': up}, 'down': {'trigger_mode': down}}
    for rows, warps in ((8, 4), (16, 4), (16, 8)):
        for ig in (1, 2, 4):
            for ir in (1, 2, 4):
                choices[f'd_r{rows}w{warps}_ig{ig}ir{ir}'] = {
                    'down': {'rows': rows, 'warps': warps, 'integer_groups': ig, 'integer_rows': ir}}
    for rows in (32, 64):
        for ig in (1, 2, 4):
            for ir in (1, 2, 4):
                choices[f'u_r{rows}_ig{ig}ir{ir}'] = {
                    'up': {'rows': rows, 'integer_groups': ig, 'integer_rows': ir}}
    return choices
...
def main():
    ...
    path = RESULTS/f'mlp_schedule_{args.tag}.json'
    if path.exists():
    ...
    result = {'codebooks': 32, 'layers': args.layers, 'configs': choices,
        ...
        'method': 'Selected G32 norm/gate/up -> down -> following clustered QKV/head preparation. '
                  'Selected eight-row down control versus producer release and integer layouts; scales preload before the original PDL wait. '
        ...
        for name, options in choices.items():
            try:
                ...
                result['resources'][name] = {stage: {'registers': kernel.n_regs,
                    ...
                    'cubin_sha256': hashlib.sha256(kernel.asm['cubin']).hexdigest()} for stage, kernel in kernels.items()}
                for _ in range(2):
                    candidate(ring[0])
