"""Full-stream CUDA register/resource checks against selected exact gate/up."""
import argparse
import hashlib
import json
import statistics
...
def main():
    ...
    path=RESULTS/f'gateup_resources_paired_{a.tag}.json';assert not path.exists(),'Preserve evidence'
    ...
    if a.profile:
        ...
        prof.export_chrome_trace(str(RESULTS/f'gateup_resources_profile_{a.tag}_trace.json'))
        (RESULTS/f'gateup_resources_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    engine.codec.close()
