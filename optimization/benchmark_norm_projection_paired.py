"""Alternate Separate/fused norm-projection graphs in one model/process.
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--pairs',type=int,default=10);p.add_argument('--tag',required=True);p.add_argument('--projections',nargs='+',choices=('qkv','up'),default=['qkv','up']);args=p.parse_args()
    ...
    if (RESULTS/f'norm_projection_paired_{args.tag}.json').exists():raise FileExistsError('Preserve previous results')
    ...
    fast.norm_projection_fused={name:dict(SELECTED[name]) for name in args.projections}
    ...
    result={'method':'One model/process, separately captured complete decode graphs and 128/256/512 buckets; reversed order every pair, same seed/reference/text per pair. One warmup pair excluded. Full streaming generation, all 32 codebooks, BF16 prefill, FP32 codec, cached cloned voice; network excluded.',
        'codebooks':32,'rows':rows,'all_full_float32_pcm_exact':True,'control_ttfa':stats([r['records']['control']['ttfa_ms'] for r in rows]),label+'_ttfa':stats([r['records'][label]['ttfa_ms'] for r in rows]),'median_paired_gain_ms':statistics.median(r['gain_ms'] for r in rows),'torch':torch.__version__,'change':'Fuse residual/RMSNorm/G32 activation quantization into selected consumer projections.','projections':args.projections}
    print(json.dumps({k:v for k,v in result.items() if k!='rows'},indent=2),flush=True)
    (RESULTS/f'norm_projection_paired_{args.tag}.json').write_text(json.dumps(result,indent=2)+'\n')
    engine.codec.close()
