"""Compare selected bulk hints with QKV-only address specialization."""
import argparse
import contextlib
import hashlib
import json
...
from .benchmark_bulk_prefetch_paired import experimental_bindings as selected_bindings
...
def experimental_bindings():
    ...
    base_norm=configured('norm',divisor=16)
    qkv_candidate=address_configured('norm',divisor=16,address_mode=1)
    def norm_candidate(*args,**kwargs):
        return (base_norm if kwargs.get('fused',False) else qkv_candidate)(*args,**kwargs)
...
def main():
    ...
    path=RESULTS/f'bulk_address_paired_{a.tag}.json';assert not path.exists(),'Preserve measurements'
    with selected_bindings():
    ...
    for pair in range(-1,a.pairs):
        ...
        for name in order:
            ...
            with experimental_bindings() if name=='candidate' else selected_bindings():
    ...
    result={'codebooks':32,'rows':rows,'checks':checks,'all_pcm_exact':True,'all_final_rng_exact':True,
        ...
        'method':'One selected G32/PDL/clocked-codec/initial-audio-graph model, separate control/candidate graph sets. Both use selected first-1/16 hints; candidate removes redundant zero-lookahead modulo only in QKV. Gate/up, down and attention-output projections are unchanged. Alternating full cached-voice requests, one excluded warmup pair. Global function bindings are scoped to this isolated benchmark process and restored; no serving change or profiler during timing.'}
    ...
    if a.profile:
        ...
        prof.export_chrome_trace(str(RESULTS/f'bulk_address_profile_{a.tag}_trace.json'))
        (RESULTS/f'bulk_address_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    engine.codec.close()
