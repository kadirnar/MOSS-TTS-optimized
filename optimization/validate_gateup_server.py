"""Smoke-check the production server's opt-in gate/up path over loopback HTTP."""
import argparse
import base64
import hashlib
import json
...
def main():
    ...
    path = RESULTS/f'gateup_server_smoke_{args.tag}.json'
    log = RESULTS/f'gateup_server_smoke_{args.tag}.log'
    ...
    command = [sys.executable, '-m', 'optimization.server', '--port', '18084',
        ...
        '--qkv-cluster', '--down-tile8', '--gateup-compiler']
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    pass
                time.sleep(.5)
            assert health['codebooks'] == 32 and health['gateup_compiler'] and health['down_tile8']
