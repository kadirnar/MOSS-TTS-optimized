"""Exact clustered gate/up -> selected eight-row down -> clustered QKV experiment."""
import argparse
import hashlib
import json
import statistics
...
def main():
    ...
    path = RESULTS/f'gateup_cluster_{args.tag}.json'
    if path.exists():
    ...
    result = {'codebooks': 32, 'layers': args.layers, 'configs': choices,
        ...
        'method': 'Selected G32 norm/gate/up -> down -> following clustered QKV/head preparation. '
                  'Selected eight-row down control versus clustered gate/up. Isolated Triton 3.8 binaries run in selected 3.7 host. '
