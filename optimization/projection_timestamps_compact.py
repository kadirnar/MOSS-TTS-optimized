...
@gluon.jit
def _wait_stamp(T,STRIDE:gl.constexpr,PDL:gl.constexpr):
    block=gl.program_id(0)
    if PDL:
        gl.inline_asm_elementwise("""{
            .reg .u32 tid; .reg .pred lane,sample,p; .reg .u64 begin,end;
            mov.u32 tid,%tid.x;
            setp.eq.u32 lane,tid,0;
            setp.ne.u32 sample,$2,0;
            and.pred p,lane,sample;
            @p mov.u64 begin,%globaltimer;
            griddepcontrol.wait;
            @p mov.u64 end,%globaltimer;
            @p st.global.u64 [$1],begin;
            @p st.global.u64 [$1+8],end;
            mov.u32 $0,0;
        }""",'=r,l,r',[T+block*4+1,(block%STRIDE)==0],dtype=gl.int32,is_pure=False,pack=1)
    else:
        _stamp(T,1,STRIDE)
        _stamp(T,2,STRIDE)
...
def _norm(X,RES,NW,W,S,SUM,Y,OQ,OS,NY,NQ,NS,N:gl.constexpr,R:gl.constexpr,
    ...
    if MODE:_wait_stamp(T,STRIDE,PDL)
    elif PDL:wait()
