"""Sequential temporary loopback servers for the active audio-head prefix comparison.
...
def main():
    ...
    base=[sys.executable,'-u','-m','optimization.server','--port','18084',
          ...
          '--decode-buckets','--attention-quant','--native-attention','--gateup-quant','--scaled-dp4a','--norm-projection']
    for mode in ('control','fused'):
        name='http_audio_head_'+mode+'_'+tag;metrics=RESULTS/(name+'_stages.jsonl');log=RESULTS/(name+'_server.log')
        ...
        args=base+['--metrics-file',str(metrics)]+(['--audio-head-buckets'] if mode=='fused' else [])
        with log.open('w') as output:
            ...
            try:
                ...
                while True:
                    ...
                    time.sleep(.5)
                assert health['codebooks']==32 and health['scaled_dp4a'] and health['norm_projection'] and health['audio_head_buckets']==(mode=='fused')
    ...
    a=json.loads((RESULTS/f'http_audio_head_control_{tag}.json').read_text());b=json.loads((RESULTS/f'http_audio_head_fused_{tag}.json').read_text())
    ...
    result={'method':'Sequential temporary servers, control then fused; identical text/reference/seeds 501-520, all 32 codebooks. No concurrent GPU test.','all_20_full_pcm_streams_exact':same,
            'control_ttfa':a['ttfa'],'fused_ttfa':b['ttfa'],'control_fresh_single_ms':a['fresh_reference_to_pcm_ms'],'fused_fresh_single_ms':b['fresh_reference_to_pcm_ms'],
            'control_cancellation_statuses':a['cancellation_recovery_statuses'],'fused_cancellation_statuses':b['cancellation_recovery_statuses'],'temporary_servers_stopped':True}
    (RESULTS/f'http_audio_head_comparison_{tag}.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
