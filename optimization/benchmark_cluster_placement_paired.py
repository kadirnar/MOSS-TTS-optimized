"""Full-stream check of cluster placement against the qualified clustered baseline."""
import argparse
import hashlib
import json
import statistics
...
def main():
    ...
    p.add_argument('--binary-bundle',type=Path,default=RESULTS/'qkv_cluster_bundle_v6');p.add_argument('--configs',nargs='+',choices=['p2_c50','p2_cNone'],default=['p2_c50'])
    ...
    path=RESULTS/f'cluster_placement_paired_{a.tag}.json';assert not path.exists(),'Preserve evidence'
    ...
    choices={'control':{'policy':1,'carveout':None}}
    choices.update({n:{'policy':2,'carveout':50 if n=='p2_c50' else None} for n in a.configs})
    def select_dispatch(name):fast.hidden=dispatches[name]
    for name,choice in choices.items():
        fast.hidden=original_hidden
        if name=='control':options,launcher=selected['c8_t2_exact'],selected_launchers['c8_t2_exact']
        else:options,launcher=configured(a.binary_bundle,**choice)
        dispatches[name]=Hidden(fast,launcher,options);select_dispatch(name)
    ...
    if a.profile:
        ...
        prof.export_chrome_trace(str(RESULTS/f'cluster_placement_profile_{a.tag}_trace.json'))
        (RESULTS/f'cluster_placement_profile_{a.tag}.txt').write_text(prof.key_averages().table(sort_by='self_cuda_time_total',row_limit=40))
    engine.codec.close()
