"""Experimental PDL for exact Gluon output/down projections."""
import torch
import triton
...
def _projection(Q,XS,W,S,base,N:gl.constexpr,K:gl.constexpr,BG:gl.constexpr,
                R:gl.constexpr,I:gl.constexpr,F:gl.constexpr):
    ...
    w=gl.load(gl.cast(W,gl.pointer_type(gl.uint32))+ri[:,None,None]*(K//8)+pos,
    ...
    scale=gl.load(S+rf[:,None]*(K//32)+gf[None,:],(rf[:,None]<N)&(gf[None,:]<K//32),0).to(gl.float32)
...
def _kernel(Q,XS,W,S,Y,OQ,OS,N:gl.constexpr,K:gl.constexpr,BG:gl.constexpr,R:gl.constexpr,
            FUSED:gl.constexpr,WARPS:gl.constexpr,IG:gl.constexpr,IROWS:gl.constexpr,PDL:gl.constexpr,TRIGGER:gl.constexpr):
    if PDL:wait()
    ...
    value=_projection(Q,XS,W,S,base,N*2 if FUSED else N,K,BG,R,I,F).to(gl.bfloat16)
    if FUSED:
...
def linear(x,w,s,*,fused=False,rows=4,warps=4,integer_groups=1,integer_rows=1,prequantized,pdl=True,trigger_mode=1,return_kernel=False):
    ...
    compiled=_kernel[(triton.cdiv(n,rows),)](q,sx,w,s,y,oq,os,n,k,triton.next_power_of_2(k//32),rows,fused,warps,integer_groups,integer_rows,pdl,trigger_mode,num_warps=warps,launch_pdl=pdl)
